...
async def test_update_profile_passes_payload_to_crud(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.profiles_manager.crud_update_profile",
        fake_update_profile,
    )

    manager = ProfileManager(db=object())

    result = await manager.update_profile(7, ProfileUpdate(first_name="Updated"))
...
async def test_update_profile_raises_not_found_when_crud_returns_none(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await manager.update_profile(7, ProfileUpdate(first_name="Updated"))
